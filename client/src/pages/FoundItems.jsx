...
const FoundItems = () => {
  ...
  const loadFoundItems = async (customSearch = search) => {
    try {
      setLoading(true);
      setError(null);
      ...
      setError('Unable to load found items from server. Please check your backend connection.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-200 pb-6">
        <div>
          <div className="flex items-center gap-2 text-indigo-600 font-bold text-xs uppercase tracking-wider mb-1">
            ...
            <span>Recovered & Awaiting Claim</span>
          </div>
          <h1 className="text-3xl font-extrabold text-slate-900 tracking-tight">
            Found Items on Campus
          </h1>
          <p className="text-sm text-slate-500 mt-1">
            Found items waiting for their rightful owners. Check the details and claim yours!
          </p>
        </div>

        <Link
          to="/report-found"
          className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl font-bold text-sm text-white bg-indigo-600 hover:bg-indigo-700 shadow-sm transition-all self-start md:self-auto"
        >
      ...
      <div className="p-4 rounded-2xl bg-indigo-50/70 border border-indigo-100 flex items-start gap-3.5 text-indigo-900 text-xs sm:text-sm">
        <ShieldCheck className="w-5 h-5 text-indigo-600 shrink-0 mt-0.5" />
        <p className="leading-relaxed">
          <strong>Safe Reclamation Tip:</strong> When claiming an item, please be prepared to provide identifying proof (e.g. device passcode, serial number, ID card photo, or specific distinguishing markings) to verify ownership.
        </p>
      </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-2 border-t border-slate-100 text-xs sm:text-sm">
          ...
          <div>
            ...
            >
              ...
              <option value="Recovered">Recovered (Claimed)</option>
            </select>
          </div>
      ...
      {/* Error Banner */}
      {error && (
        <div className="p-4 rounded-xl bg-rose-50 border border-rose-200 text-rose-700 text-sm flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 shadow-xs">
          ...
          <button
            ...
            className="px-4 py-1.5 bg-rose-600 hover:bg-rose-700 text-white text-xs font-semibold rounded-lg transition-colors flex items-center gap-1.5 shadow-xs shrink-0 cursor-pointer"
          >
            <RefreshCw className="w-3.5 h-3.5" />
            Retry
          </button>
      ...
      {loading ? (
        ...
        <div>
          <div className="flex items-center justify-between mb-4 text-xs font-medium text-slate-500">
            <span>Total: <strong>{items.length}</strong> found {items.length === 1 ? 'item' : 'items'} available</span>
          </div>
        ...
        <EmptyState
          ...
          description="No found items currently match your search. Try changing your filters or check back later."
          actionType="found"
        />
      )}
